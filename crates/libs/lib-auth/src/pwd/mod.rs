...
/// Generate salt. for not a Uuid string
pub async fn generate_random_uuid_v4() -> Result<Uuid> {
    tokio::task::spawn_blocking(Uuid::new_v4)
        .await
        .map_err(|_| Error::FailSpawnBlockForSalt)
}
...
/// Hash the password with the default scheme synchronously.
pub fn hash_pwd_sync(to_hash: ContentToHash) -> Result<String> {
    hash_for_scheme(DEFAULT_SCHEME, to_hash)
}
